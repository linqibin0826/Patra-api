...
- Spring profiles: `dev` (默认) 用于本地 IDE 开发(基建地址走 `PATRA_INFRA_HOST` / `TAILSCALE_IP` 注册),`container` 用于 Mac mini 容器化部署(全配置走环境变量注入,由 CD 部署);`prod` 预留给未来真正的云生产。基础配置将 `SPRING_PROFILES_ACTIVE` 默认设置为 `dev`;容器部署由 `.env.common` 导出 `SPRING_PROFILES_ACTIVE=container`。
...
- 敏感值 (DB/Redis 凭证、API 密钥) 存放在环境变量中,在 `application-container.yml` 中使用 (例如: `REGISTRY_DB_URL`, `INGEST_DB_URL`)。仅提交本地引导的默认开发配置。
